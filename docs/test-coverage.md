...
- **TUI** (`src/tui/`): the whole Ink layer: `app.tsx`, every component (`dashboard`, `logs-view`, `detail-view`, `add-service`, `import-stack`, `service-table`, `status-badge`, `header`, `text-input`, `sync-view`, `alert`), `sync.tsx`, `use-daemon.ts` (including the `proxyStatus` fetch), `frame-clock.ts`, `theme.ts`, `devtools-stub.ts`.
