...
and `status_code`; exec probes accept their own `working_dir`. Probes always
target the service's port directly, never the routing proxy (a health check
shouldn't depend on the proxy being up). Liveness failure restarts the
instance.
...
and `OUTRIDER_URL` (plus `NODE_EXTRA_CA_CERTS` when TLS is on).
...
Routing is opt-in per process, served by outrider's own in-process proxy —
no external CLI or dependency. Configs that hard-code ports keep working
untouched. Route names must be unique system-wide and must be valid DNS
labels; conflicts fail the import naming both claimants. `framework` feeds
...
`routeKind: 'static'` on the registry entry so the route table reports it
distinctly from a daemon-allocated ("managed") route; see
