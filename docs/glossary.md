...
ignore the injected `PORT` — `kubectl port-forward`, `tsh proxy`, and the like.
The route then points straight at that fixed port as a static portless alias.
...
`exec` (run a command) and `http_get` (hit a URL — through the route, for routed
services, so it tests the exact path a user would).
