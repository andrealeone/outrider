...
The main view is a virtualised table of every registered service (name,
...
detail with `i`, add with `a`, import with `m`, search with `/`, sort with
`s`. The full key map is in [usage](../usage.md).
