  ...
  const dotenvPath = join(composeDir, '.env'),
    dotenv =
      !config.is_dotenv_disabled && existsSync(dotenvPath)
        ...
        : {},
    context = { ...process.env, ...dotenv, ...parseEnvList(config.environment) }
...
export const loadProject = (entry: string, options: LoadOptions = {}): LoadedProject => {
  ...
  const sources = [mainFile],
    override = discoverOverrideFile(mainFile)
...
/** Default import tag: config name, else the compose file's directory name. */
export const importTagFor = (project: LoadedProject): string => {
