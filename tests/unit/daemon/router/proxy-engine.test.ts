...
const startUpstream = (handler: (req: Request) => Response): number => {
...
describe('ProxyEngine (plain HTTP)', () => {
  test('forwards a request by Host header, restating it for the upstream', async () => {
    const upstreamPort = startUpstream(
      (req) => new Response(`host=${req.headers.get('host')}`),
    )
    routes.upsert('engine-a.localhost', upstreamPort, 'managed', 'a')
    const { port } = await startEngine()

    const res = await fetch(`http://127.0.0.1:${port}/`, { headers: { Host: 'engine-a.localhost' } })
  ...
  test('propagates the upstream status and body', async () => {
    ...
    const res = await fetch(`http://127.0.0.1:${port}/`, { headers: { Host: 'engine-b.localhost' } })
  ...
  test('502s when the upstream is unreachable', async () => {
    ...
    const res = await fetch(`http://127.0.0.1:${port}/`, { headers: { Host: 'engine-c.localhost' } })
